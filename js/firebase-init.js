...
window.MahinFirebaseReady = (async function initFirebase() {
  ...
  // Storage SDK is only loaded on pages that need file uploads (index.html).
  // Guard this so login.html / register.html / admin.html don't crash.
  window.storage = (typeof firebase.storage === "function") ? firebase.storage() : null;

  return true;
})();

// Google sign-in, shared by login.html and register.html. Creates a users/{uid}
// profile document automatically on first sign-in if one doesn't exist yet
// (phone number will be blank — the person can fill it in via the profile widget).
