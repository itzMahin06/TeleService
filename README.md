...
এই প্রজেক্টে আছে:
- `index.html` — মূল হোমপেজ (সবার জন্য উন্মুক্ত): হিরো, সাম্প্রতিক প্রোডাক্ট, এবং ম্যানুয়াল আবেদন ফর্ম (লগইন লাগবে)
- `products.html` — সকল প্রোডাক্ট (চাকরির সার্কুলার)-এর তালিকা
- `login.html`, `register.html` — লগইন/রেজিস্ট্রেশন (Firebase Auth + Google Sign-In)
- `admin.html` — অ্যাডমিন প্যানেল (প্রোডাক্ট CRUD, অর্ডার তালিকা + অনুমোদন/বাতিল + প্রিমিয়াম ইনভয়েস, ব্যবহারকারী তালিকা + দেখা/সম্পাদনা/মুছা)
...
- `js/firebase-init.js` — সব পেজের জন্য শেয়ার্ড Firebase সেটআপ + Google Sign-In হেল্পার
- `js/profile-widget.js` — হেডারের প্রোফাইল আইকন + নিজের তথ্য দেখা/সম্পাদনা করার মডাল (ইউজার ও অ্যাডমিন দুজনের জন্যই)
- `js/products-widget.js` — প্রোডাক্ট কার্ড রেন্ডার করে (হোমপেজ ও `products.html` দুই জায়গাতেই ব্যবহৃত)
- `js/order-popup.js` — "অর্ডার করুন" পপ-আপ ফর্ম (মন্ত্রণালয় ও ফি অটো-ফিল হয়ে থাকে, শুধু ইউজার আইডি + পেমেন্ট তথ্য দিতে হয়)
...
- প্রোডাক্ট (চাকরির সার্কুলার) → Firestore `products` কালেকশন — সবাই দেখতে পারে, শুধু অ্যাডমিন যোগ/সম্পাদনা/মুছতে পারে
- আবেদন/অর্ডার → Firestore `orders` কালেকশন (মন্ত্রণালয়ের নাম ও ইউজার আইডি সহ — PDF আপলোডের দরকার নেই, তাই Firebase Storage ব্যবহার করা হয় না)
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /users/{userId} {
      ...
      // ব্যবহারকারী নিজে নিজের নাম/ফোন এডিট করতে পারবেন, অ্যাডমিনও এডিট করতে পারবেন
      allow update: if request.auth != null &&
                       (request.auth.uid == userId || isAdmin());
      // শুধু অ্যাডমিন প্রোফাইল ডিলিট করতে পারবে (এটি শুধু Firestore প্রোফাইল মুছে —
      // Firebase Authentication-এর লগইন অ্যাকাউন্ট আলাদাভাবে Console থেকে মুছতে হবে)
    ...
    match /products/{productId} {
      // প্রোডাক্ট সবাই (এমনকি লগইন না করা ভিজিটরও) দেখতে পারবে
      allow read: if true;
      // শুধু অ্যাডমিন যোগ/সম্পাদনা/মুছতে পারবে
...
> 📌 **অর্ডার পপ-আপে "হোয়াটসঅ্যাপ/টেলিগ্রাম" বাটন** — `js/order-popup.js` ফাইলের একদম উপরে `TELEGRAM_USERNAME = "MahinServiceBD"` লেখা আছে, এটা একটা প্লেসহোল্ডার। এটা বদলে আপনার আসল টেলিগ্রাম ইউজারনেম বসিয়ে দিন, নাহলে টেলিগ্রাম বাটনে ক্লিক করলে ভুল জায়গায় যাবে। `WHATSAPP_NUMBER` আগে থেকেই আপনার নম্বর (01931923910) দিয়ে ঠিক করা আছে।

> ⚠️ **এই আপডেটে Firestore Rules পরিবর্তন করা আবশ্যক** — নতুন `products` কালেকশন এবং অ্যাডমিনের ইউজার এডিট/ডিলিট ফিচার কাজ করার জন্য উপরের সম্পূর্ণ রুলসটা Firebase Console-এ **Publish** করতে হবে, শুধু আগেরটা রেখে দিলে হবে না।
